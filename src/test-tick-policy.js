...
assert(startup.includes('diagnostic goal, not a mandatory checklist'), 'startup self-check is a goal rather than a fixed script')
assert(startup.includes('which checks, if any'), 'startup self-check allows the model to judge whether checks are useful')
assert(startup.includes('complete_startup_self_check'), 'startup self-check still has an explicit way to close state')
assert(!startup.includes('write_file') && !startup.includes('hotspot_mode') && !startup.includes('media_mode'), 'startup policy does not prescribe concrete tools')
assert(!startup.includes('early awakening period'), 'startup diagnostic context takes precedence over awakening context')
