...
export function setTyphoonMode(visible, { source = 'brain-ui' } = {}) {
  ...
  window.dispatchEvent(new CustomEvent('nanai:typhoon-mode', { detail: { active: next } }))
...
export function initTyphoon() {
  ...
  window.addEventListener('nanai:hotspot-mode', (event) => { if (event?.detail?.active && active) setTyphoonMode(false, { source: 'hotspot_open' }) })
  window.addEventListener('nanai:worldcup-mode', (event) => { if (event?.detail?.active && active) setTyphoonMode(false, { source: 'worldcup_open' }) })
