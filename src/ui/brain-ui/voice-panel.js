...
// 暴露 initVoicePanel + window.nanaiVoice（承重墙：app.js 的 TTS 打断与视频/音乐联动依赖它）。
//
// 解耦结构：
...
export function initVoicePanel({
  ...
  // ─── 承重墙：window.nanaiVoice 接口契约（app.js 依赖，不可改形状） ───
  window.nanaiVoice = {
  ...
  window.addEventListener('nanai:video-mode', (event) => {
    if (event.detail?.active) {
      window.nanaiVoice.suspendForMedia();
    } else {
      window.nanaiVoice.resumeAfterMedia();
    }
  });

  window.addEventListener('nanai:music-mode', (event) => {
    if (event.detail?.active) {
      window.nanaiVoice.suspendForMedia();
    } else {
      window.nanaiVoice.resumeAfterMedia();
