...
const ACTIVATION_WARMUP_KEY = "nanai_activation_warmup_until";
const UI_ZOOM_STORAGE_KEY = "nanai_ui_zoom_factor";
...
const MEMORY_GRAPH_STORAGE_KEY = "nanai-memory-graph-enabled";
...
const IGNORED_VERSION_KEY = "nanai_ignored_update_version";
const SUPPRESS_UPDATES_KEY = "nanai_suppress_update_notifications";
...
function handle({ type, data = {} }) {
  switch (type) {
    ...
    case "agent_name_updated":
      ...
      break;
    case "media_mode":
      window.dispatchEvent(new CustomEvent("nanai:media", { detail: data }));
      break;
    case "aivideo_mode":
      window.dispatchEvent(new CustomEvent("nanai:aivideo", { detail: data }));
      break;
    case "hotspot_mode":
    ...
    case "person_card_mode":
      ...
      break;
    case "social_status":
      window.dispatchEvent(new CustomEvent("nanai:social_status", { detail: data }));
      break;
...
const TTS_STREAMING_KEY = 'nanai.tts.streaming';
...
function activateTTSAudioGraph(graph) {
  ...
  window.nanaiVoice?.setTTSAnalyser?.(ttsAudioGraph?.analyser || null);
}
...
function clearTTSAudioGraph(graph) {
  ...
  window.nanaiVoice?.setTTSAnalyser?.(null);
}
...
function startTTSAudio(audioEl, revokeUrl, opts = {}) {
  ...
  if (manageMic) window.nanaiVoice?.suspendForTTS?.();
  ...
  const finish = () => {
    ...
    ttsCurrentText = '';
    if (manageMic) window.nanaiVoice?.resumeAfterMedia();
  };
...
async function playTTSReply(text) {
  ...
  try {
    ...
    clearTTSAudioGraph();
    ttsCurrentText = '';
    window.nanaiVoice?.resumeAfterMedia();
  }
}
...
async function pumpSttsQueue() {
  ...
  if (!sttsMicSuspended) { sttsMicSuspended = true; window.nanaiVoice?.suspendForTTS?.(); }
  const onComplete = () => {
    sttsSpoken += seg;
    sttsCurSeg = '';
    sttsPlaying = false;
...
function endStreamingTTS() {
  ...
  clearTTSAudioGraph();
  if (sttsMicSuspended) { sttsMicSuspended = false; window.nanaiVoice?.resumeAfterMedia(); }
...
(function initSettings() {
  ...
  const VOICE_LANG_KEY       = "nanai-voice-lang";
  const VOICE_AUTO_SEND_KEY  = "nanai-voice-auto-send";
  const VOICE_AUTO_MIC_KEY   = "nanai-voice-auto-mic";
  const VOICE_THRESHOLD_KEY  = "nanai-voice-threshold";
  const VOICE_PROVIDER_KEY   = "nanai-voice-provider";
  const VOICE_MIC_DEVICE_KEY = "nanai-voice-mic-device-id";
  ...
  if (saveVoiceBtn) {
    saveVoiceBtn.addEventListener("click", async () => {
      ...
      window.dispatchEvent(new CustomEvent("nanai:voice-threshold", { detail: { threshold } }));
  ...
  window.addEventListener("nanai:social_status", (e) => {
    const d = e.detail;
...
initVoicePanel({
  ...
  getLang:       () => localStorage.getItem("nanai-voice-lang") || "zh-CN",
  getAutoSend:   () => localStorage.getItem("nanai-voice-auto-send") !== "false",
  getAutoMic:    () => localStorage.getItem("nanai-voice-auto-mic") === "true",
});
...
(function initMediaModes() {
  ...
  function setPanelVisible(visible) {
    ...
    window.dispatchEvent(new CustomEvent("nanai:video-mode", {
  ...
  function setMusicPanelVisible(visible) {
    ...
    window.dispatchEvent(new CustomEvent("nanai:music-mode", {
  ...
  window.nanaiMedia = { handle: handleMediaCommand, showVideo, controlVideo, showImage, showCamera, showMusic, controlMusic };
  window.addEventListener("nanai:media", (event) => handleMediaCommand(event.detail || {}));
  ...
  (() => {
    ...
    window.addEventListener("blur", () => {
      if (!pttHeld) return;
      pttHeld = false;
      window.nanaiVoice?.pttEnd?.({ send: false });
    });
  })();
...
(function initAIVideoMode(){
  ...
  function setActive(on){
    ...
    if(active){ try{ window.nanaiMedia&&window.nanaiMedia.controlVideo&&window.nanaiMedia.controlVideo({action:"pause"}); }catch(e){} document.body.classList.remove("video-mode"); }
  ...
  window.addEventListener("nanai:aivideo", function(e){ handle(e.detail||{}); });
  window.nanaiAIVideo={ handle:handle, open:openPanel, close:closePanel };
