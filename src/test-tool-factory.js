...
// Approved proposal: review -> install -> callable.
{
  ...
  const result = await executeInstalledTool(goodName, { text: 'nanai' })
  assert(result === 'NANAI', 'installed generated tool executes through marketplace registry', result)
...
// Direct install keeps permission semantics distinct from the managed gate.
{
  ...
  try {
    ...
    `, {
      NANAI_USER_DIR: tempUserDir,
      NANAI_RESOURCES_DIR: process.cwd(),
    })
