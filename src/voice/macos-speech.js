...
export function createMacSpeechSession(config = {}, onTranscript, onError, onClose, onStatus) {
  ...
  const child = spawn(resolved.cmd, [...resolved.args, '--lang', lang, '--mode', mode, '--input', 'stdin'], {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (msg.type === 'transcript' && msg.text) {
          ...
          onStatus?.(msg.message || 'macOS 本地语音识别已就绪')
  ...
  child.stdin?.on('error', (err) => {
    if (!closed && err.code !== 'EPIPE') onError(`macOS 本地语音写入失败: ${err.message}`)
  })
  ...
  return {
    sendAudio(pcmBuffer) {
      if (closed || !pcmBuffer?.length || child.stdin?.destroyed) return
      try { child.stdin.write(Buffer.from(pcmBuffer)) } catch {}
    },
    flush() {},
    close() {
      if (closed) return
