...
function getApiHost() {
  const envHost = String(globalThis.process?.env?.NANAI_HOST || '').trim()
...
function isLanAccessEnabled() {
  return getNetworkConfig().allowLanAccess
    || /^(1|true|yes|on)$/i.test(String(globalThis.process?.env?.NANAI_ALLOW_LAN || '').trim())
}
...
function getAuthToken() {
  return String(globalThis.process?.env?.NANAI_API_TOKEN || '').trim()
}
