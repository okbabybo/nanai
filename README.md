...
> 🍃 **完全开源 · 永久免费 · 无需任何激活码或付费解锁**
> 你只需要准备一个**自己**的大模型 API Key（DeepSeek / MiniMax / OpenAI 等任选），奈奈本身不收取任何费用。
...
> 🏛 **架构来源**：Fork 自上游开源桌面 AI Agent 项目（保留其完整架构、工具系统、记忆引擎、Brain UI 与 Electron 集成）

项目由 Electron 桌面壳、本地 HTTP 服务、LLM 调用层、记忆系统、工具执行器、语音系统、社交连接器和 Brain UI 组成。奈奈在此基础上，用更柔和的视觉与更温柔的语气，为每一个普通用户带来一个**住在桌面里、记得你说过的每句话、会在合适的时候主动伸手**的数字伙伴。

## 技术血脉

奈奈是一个**品牌个性化分支**，架构完全来自上游开源桌面 AI Agent 工程。两者的核心架构 1:1 沿用：
...
| Brain UI 骨架 | ✅ 沿用 | ✅ 沿用 |
| Electron 集成 | ✅ 沿用 | ✅ 沿用 |
| 视觉风格 | 深蓝黑 + 橙（武学冷峻） | 粉色长发 + 治愈系（清新可爱） |
| 人格语气 | 克制专业 | 温和耐心 |
| 目标用户 | 开发者 / 高级用户 | C 端普通用户 |

**上游仓库**：<https://github.com/xiaoyuanda666-ship-it/Nanai>（致谢开源贡献 🙏）
...
Nanai 的长期状态主要保存在本地 SQLite 数据库中，包括：
