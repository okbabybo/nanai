...
项目由 Electron 桌面壳、本地 HTTP 服务、LLM 调用层、记忆系统、工具执行器、语音系统、社交连接器和 Brain UI 组成。奈奈在白龙马的硬核工程底座上，用更柔和的视觉与更温柔的语气，为每一个普通用户带来一个**住在桌面里、记得你说过的每句话、会在合适的时候主动伸手**的数字伙伴。

## 与白龙马的关系

奈奈是白龙马的**品牌个性化 Fork**，而非重写。两者的核心架构完全一致：
...
| Brain UI 骨架 | ✅ 沿用 | ✅ 沿用 |
| Electron 集成 | ✅ 沿用 | ✅ 沿用 |
| 视觉风格 | 深蓝黑 + 橙（武学冷峻） | 薄荷绿 + 奶茶 + 米白（清新治愈） |
| 人格语气 | 克制专业 | 温和耐心 |
